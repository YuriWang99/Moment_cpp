	...
	FString RequestURL, const FRDTGetStringDelegate& CallOnComplete, const FRuntimeDataTableOperationParams OperationParams)
{
	...
	Request->OnProcessRequestComplete().BindUObject(this, &URuntimeDataTableObject::OnResponseReceived_GET_SheetAsCSV, CallOnComplete);
...
void URuntimeDataTableObject::OnResponseReceived_GET_SheetAsCSV(
	FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful, FRDTGetStringDelegate CallOnComplete)
{
		...
		for (int32 i = 0; i < ArrayHelper.Num(); i++)
		{
			...
			for (int32 x = 0; x < CsvInfo.CSV_Headers.Num(); x++)
			{
				...
				if (Property)
				{
					...
					if (ValueAsString != "")
					{
						//FRuntimeDataTableModule::Print("Count: " + FString::FromInt(Count) + ", ValueAsString is: " + ValueAsString, "debug");
